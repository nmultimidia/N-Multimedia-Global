...
router.post("/diagnostic", async (req, res) => {
  const { name, email, budget, timeline, need, countryCode } = req.body;
  ...
  const [row] = await db
    ...
    .values({ name, email, budget, timeline, need, countryCode: countryCode || "unknown", status: "new" })
    .returning();

  sendDiagnosticEmail({ name, email, budget, timeline, need, countryCode }).catch((err) =>
